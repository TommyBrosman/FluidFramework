...
| `lz4js` compress+decompress (12.7 KiB) + `OpCompressor` + `OpDecompressor` | **~15,000 B** | ⏸️ **GATED — session-wide wire guarantee.** Op compression is **ON by default** (grouped batching enabled ⇒ `minimumBatchSizeInBytes: 614400`, `compressionDefinitions.ts:42`). `lz4js` (a CJS module, so compress and decompress cannot be tree-shaken apart) ships ~12.7 KiB. `decompress` is required on the inbound hot path because **any participant** in a shared session may send a batch > ~600 KB as a compressed op. Removing it needs a guarantee that *no client in any session* ever exceeds the compression threshold (or that compression is disabled session-wide) — a wire-format/interop assumption, not a per-client feature opt-in. Riskier than the landed stubs; not landed without that guarantee. | Wire-format interop (session-wide) |
